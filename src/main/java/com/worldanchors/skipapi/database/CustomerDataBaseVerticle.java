...
public class CustomerDataBaseVerticle extends AbstractVerticle {
    ...
    public static final String CONFIG_QUEUE = "skip.customer.queue";
    ...
    public void start(Future<Void> startFuture) throws Exception {
        ...
        .put("max_pool_size", config().getInteger(CONFIG_JDBC_MAX_POOL_SIZE, 30)),
                config().getString(CONFIG_JDBC_SHARED_POOL_NAME, "skip.customer.pool")
        );

        DatabaseServices.create(CustomerDatabaseServices.class.getName(),jdbcClient, ready ->{
            if(ready.succeeded()){
